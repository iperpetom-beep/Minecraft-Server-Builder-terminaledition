...
- ✅ Автозагрузка последнего сервера при запуске (можно включить в настройках)
...
3. В менеджере: `3) Настройки` → `6) Настройки playit.gg`
4. Введите `Secret key` — claim code или secret
5. Вернитесь в главное меню и включите: `4) Использовать playit.gg`

### Автозагрузка

В настройках (`3) Настройки` → `5) Автозагрузка последнего сервера`) можно включить автоматический запуск последнего сервера при старте приложения.
...
Если включена автозагрузка в настройках, при запуске приложения будет 10-секундный таймер для автоматического запуска последнего сервера. Нажмите Enter для отмены автозагрузки.
...
3. Укажите объём RAM
4. Сервер запустится, playit туннель (если включён) запустится автоматически
5. При остановке сервера (Ctrl+C) туннель также остановится
